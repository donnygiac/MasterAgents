...
- **Schema attuale**: leggi lo schema reale del progetto (Prisma: individua `schema.prisma` con
  `codegraph files` o glob `**/schema.prisma`; SQL puro: le migrazioni) — ogni proposta parte
  dallo stato reale, mai da memoria.
- **Chi usa le tabelle coinvolte**: dal backend, `codegraph callers <metodoService>` sui service
  che leggono/scrivono quelle tabelle — i query path reali si ricavano da lì, non si inventano.
...
> Vincoli di dominio: leggi la sezione "Vincoli di dominio" di CLAUDE.md prima di proporre
> qualunque forma — le regole lì elencate non sono negoziabili.
...
- **Input atteso**: design doc di `brainstorming` con sezione "Impatto schema" ≠ nessuno,
  oppure un problema di performance/integrità su tabelle esistenti.
- **Output prodotto**: DDL come proposta testuale (mai eseguita) + storico in
  `.claude/skills/designing-mysql-tables/ideas/<progetto>.md`. L'applicazione allo schema
  e le migrazioni restano decisioni dell'utente.
- **Prossima skill**: implementazione (service/controller) con `managing-error-patterns` per i
  codici errore dei nuovi vincoli (es. unicità → errore di conflitto 409); a valle,
  `architecture-analysis` per aggiornare l'ERD documentato.

## Resources
- Consulta [tech-stack.md](../maintaining-brand-identity/resources/tech-stack.md) per i vincoli di stack.
- Consulta [managing-error-patterns](../managing-error-patterns/SKILL.md) per resilienza DB.
